...
public interface MainRepository extends JpaRepository<Main, Long> {
    ...
    @Query(value = "SELECT u.id, u.name FROM main_office u WHERE is_delete <> 1", nativeQuery = true)
    public List<MainInterface> findAllMain();

    @Query(value = "SELECT u.id, u.name FROM main_office u WHERE u.id = :mainId AND is_delete <> 1", nativeQuery = true)
