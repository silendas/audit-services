...
public interface PagAuditWorkingPaper extends PagingAndSortingRepository<AuditWorkingPaper, Long> {
    ...
    @Query(value = "SELECT * FROM audit_working_paper awp WHERE awp.created_at BETWEEN :start_date AND :end_date AND awp.is_delete <> 1;", nativeQuery = true)
    Page<AuditWorkingPaper> findWorkingPaperInDateRange(@Param("start_date") Date start_date,
            @Param("end_date") Date end_date, Pageable pageable);

    @Query(value = "SELECT awp.* FROM audit_working_paper awp INNER JOIN users us ON awp.user_id=us.id WHERE us.fullname LIKE :name AND awp.is_delete <> 1;", nativeQuery = true)
    Page<AuditWorkingPaper> findWorkingPaperByName(@Param("name") String name, Pageable pageable);

    @Query(value = "SELECT * FROM audit_working_paper awp WHERE awp.branch_id= :id AND awp.is_delete <> 1;", nativeQuery = true)
    Page<AuditWorkingPaper> findWorkingPaperByBranch(@Param("id") Long id, Pageable pageable);

    @Query(value = "SELECT awp.* FROM audit_working_paper awp INNER JOIN users us ON awp.user_id=us.id WHERE us.fullname LIKE :name AND awp.branch_id = :id AND awp.is_delete <> 1;", nativeQuery = true)
    Page<AuditWorkingPaper> findWorkingPaperByNameAndBranch(@Param("name") String name,@Param("id") Long id, Pageable pageable);

    @Query(value = "SELECT awp.* FROM audit_working_paper awp INNER JOIN users us ON awp.user_id=us.id WHERE us.fullname LIKE :name AND awp.branch_id = :id AND awp.created_at BETWEEN :start_date AND :end_date AND awp.is_delete <> 1;", nativeQuery = true)
    ...
    @Query(value = "SELECT awp.* FROM audit_working_paper awp INNER JOIN users us ON awp.user_id=us.id WHERE us.fullname LIKE :name AND awp.created_at BETWEEN :start_date AND :end_date AND awp.is_delete <> 1;", nativeQuery = true)
    Page<AuditWorkingPaper> findWorkingPaperByNameAndDate(@Param("name") String name,@Param("start_date") Date start_date,
    @Param("end_date") Date end_date, Pageable pageable);

    @Query(value = "SELECT awp.* FROM audit_working_paper awp WHERE awp.branch_id = :id AND awp.created_at BETWEEN :start_date AND :end_date AND awp.is_delete <> 1;", nativeQuery = true)
