...
public interface FollowUpRepository extends JpaRepository<FollowUp, Long> {
    ...
    @Query(value = "SELECT c.report_number, c.code, EXTRACT(YEAR FROM c.created_at) as created_year FROM follow_up c WHERE c.updated_by = :userId AND c.is_delete = 0 ORDER BY c.id DESC LIMIT 1;", nativeQuery = true)
    Optional<NumberClarificationInterface> checkNumberFollowUpInput(@Param("userId") Long id);

    @Query(value = "SELECT c.* FROM follow_up c WHERE c.clarification_id = :id LIMIT 1;", nativeQuery = true)
