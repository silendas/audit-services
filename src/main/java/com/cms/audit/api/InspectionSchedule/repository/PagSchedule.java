...
public interface PagSchedule extends PagingAndSortingRepository<Schedule, Long> {
        ...
        @Query(value = "SELECT * FROM inspection_schedule u WHERE u.user_id = :userId AND u.category = :category  AND u.is_delete <> 1 AND u.status <> 'REQUEST' AND u.status <> 'APPROVE' AND u.status <> 'REJECTED' ORDER BY u.id ASC;", nativeQuery = true)
        ...
        @Query(value = "SELECT u.* FROM inspection_schedule u WHERE u.user_id = :userId AND u.category = :category AND (u.start_date BETWEEN :start_date AND :end_date OR u.end_date BETWEEN :start_date AND :end_date) AND u.is_delete <> 1 AND u.status <> 'REQUEST' AND u.status <> 'APPROVE' AND u.status <> 'REJECTED' ORDER BY u.id ASC;", nativeQuery = true)
