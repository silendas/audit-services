...
public interface PagAuditDailyReportDetail extends PagingAndSortingRepository<AuditDailyReportDetail, Long> {
    ...
    @Query(value = "SELECT u.* FROM audit_daily_report_detail u INNER JOIN audit_daily_report pr ON u.audit_daily_report_id=pr.id WHERE u.audit_daily_report_id = :id AND u.is_delete <> 1 ORDER BY u.id ASC ", nativeQuery = true)
    Page<AuditDailyReportDetail> findByUserId(@Param("id") Long id, Pageable pageable);

    @Query(value = "SELECT u.* FROM audit_daily_report_detail u INNER JOIN audit_daily_report pr ON u.audit_daily_report_id=pr.id WHERE u.audit_daily_report_id = :id AND (u.created_at BETWEEN :start_date AND :end_date) AND u.is_delete <> 1 ORDER BY u.id ASC ", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM audit_daily_report_detail u WHERE AND (u.created_at BETWEEN :start_date AND :end_date) AND u.is_delete <> 1 ORDER BY u.id ASC ;", nativeQuery = true)
    Page<AuditDailyReportDetail> findAllLHADetailByDate(@Param("start_date") Date start_date,
    @Param("end_date") Date end_date, Pageable pageable);

    
    @Query(value = "SELECT * FROM audit_daily_report_detail u WHERE AND (u.created_at BETWEEN :start_date AND :end_date) AND u.audit_daily_report_id = :id AND u.is_delete <> 1 ORDER BY u.id ASC ;", nativeQuery = true)
