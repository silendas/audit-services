...
public interface AuditDailyReportRepository extends JpaRepository<AuditDailyReport, Long> {
        ...
        @Query(value = "SELECT u.* FROM audit_daily_report u INNER JOIN users us ON u.user_id=us.id WHERE us.fullname LIKE '% :name %' AND u.created_at BETWEEN :start_date AND :end_date ;", nativeQuery = true)
        ...
        @Query(value = "SELECT u.* FROM audit_daily_report u INNER JOIN branch_office bo ON u.branch_id = bo.id INNER JOIN area_office ao ON bo.area_id = ao.id INNER JOIN region_office ro ON ao.region_id=ro.id WHERE ro.id = :id AND u.user_id = :user_id AND u.created_at BETWEEN :start_date AND :end_date;", nativeQuery = true)
